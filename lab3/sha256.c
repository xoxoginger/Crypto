...
void sha256_final(SHA256_CTX* ctx, BYTE hash[], size_t times) {
    ...
    if (ctx->datalen < 56) {
        ...
        ctx->data[i++] = 0x80; while (i < times) // !
            ...
            ctx->data[i++] = 0x00; sha256_transform(ctx, ctx->data, times); memset(ctx->data, 0, 56);
    ...
    ctx->data[59] = ctx->bitlen >> 32; ctx->data[58] = ctx->bitlen >> 40; ctx->data[57] = ctx->bitlen >> 48; ctx->data[56] = ctx->bitlen >> 56; sha256_transform(ctx, ctx->data, times);
